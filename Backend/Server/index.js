...
app.get('/imageUserLogin/:dadesUserLogIn', (req, res) => {
  ...
  con.query("SELECT UPLOADS.path FROM PERSONA JOIN UPLOADS ON (PERSONA.id_image = UPLOADS.id_upload) WHERE PERSONA.user = '" + user +"';", function (err, result, fields) {
    ...
    if (array[2] == null){
      res.send("http://192.168.1.34:5501/Backend/Server/uploads/user_images/normal_user_img.jpg");
    } else {
      res.send("http://192.168.1.34:5501/Backend/Server" + fileName);
    }    
  });
});
...
app.get("/dadesProductsJSON", (req, res) => {
  con.query("SELECT PRODUCTE.id_producte, PRODUCTE.nom, PRODUCTE.preu, PRODUCTE.descripcion, PRODUCTE.categoria, CONCAT('http://192.168.1.34:5501/Backend/Server/',UPLOADS_PRODUCT.path) as path, PERSONA.user, PRODUCTE.estado_prod FROM PRODUCTE JOIN UPLOADS_PRODUCT ON (PRODUCTE.id_image = UPLOADS_PRODUCT.id_upload) JOIN PERSONA ON (PRODUCTE.id_usu = PERSONA.id) ORDER BY PRODUCTE.id_producte", function(err, result, field){
    res.send(result);
  });
});
